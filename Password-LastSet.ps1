...
## Script
Get-ADUser -Filter {(Enabled -eq "True")} | ? { ($_.distinguishedname -notlike '*DSADMINS*') -or ($_.DistinguishedName -notlike "Builtin*") } -Properties Office,DisplayName,PasswordLastSet | Where {$_.PassWordLastSet -le (Get-Date).AddDays(-$Days) } | Select-Object Office,DisplayName,PasswordLastSet | Out-File $env:USERPROFILE\Desktop\$Savefile.csv
